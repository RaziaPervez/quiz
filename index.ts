...
console.log(chalk.bgBlue("\n\tWelcome to TYPESCRIPT QUIZ App \n"));
    ...
    [
        {
            name:"answer",
            type:"list",
            message:chalk.yellowBright("Q2. Which command is used to make tsconfig file?"),
            choices:[
...
console.log(chalk.yellow.bold(`No. of Correct Answers = ${marksGain}`));
console.log(chalk.yellow.bold(`No. of Wrong Answers = ${marksLose *-1 }`));

if (marksGain > marksLose* -1){
    console.log(chalk.greenBright.bold("Congratulations ! You Won The Adventure Quiz"));
}
else{
    console.log(chalk.red.bold("OOPS ! You Lost The Adventure Quiz"));
}